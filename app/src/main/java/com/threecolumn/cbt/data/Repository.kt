...
class ThoughtRecordRepository(private val dao: ThoughtRecordDao) {
    ...
    suspend fun getAllOnce(): List<ThoughtRecord> = dao.getAllOnce()
    ...
    suspend fun saveAll(records: List<ThoughtRecord>) = dao.insertAll(records)
...
class JournalEntryRepository(private val dao: JournalEntryDao) {
    ...
    suspend fun getAllOnce(): List<JournalEntry> = dao.getAllOnce()
    suspend fun getById(id: Long): JournalEntry? = dao.getById(id)
    suspend fun save(entry: JournalEntry): Long = dao.upsert(entry)
    suspend fun saveAll(entries: List<JournalEntry>) = dao.insertAll(entries)
